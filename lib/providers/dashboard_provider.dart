...
class DashboardProvider extends ChangeNotifier {
  ...
  // Getters
  int get totalTrainingSites =>
      _totalTrainingSites; // Returns synced count only
  ...
  Future<void> loadDashboardData() async {
    ...
    try {
      ...
      final results = await Future.wait([
        _trainingSiteRepository
            .getSyncedCount(), // Total count = only synced records
        _trainingSiteRepository.getUnsyncedCount(),
        _beneficiaryRepository
            .getSyncedCount(), // LOCAL SAVED = synced records only
        _beneficiaryRepository
            .getUnsyncedCount(), // OFFLINE SAVED = unsynced records
      ...
      developer.log(
          'Training Sites - Total (Synced): $_totalTrainingSites, Unsynced: $_unsyncedTrainingSites',
          name: 'DashboardProvider');
      developer.log(
          'Beneficiaries - Total: $_totalBeneficiaries, Unsynced: $_unsyncedBeneficiaries',
          name: 'DashboardProvider');
      developer.log(
          'Trainings - Total: $_totalTrainings, Unsynced: $_unsyncedTrainings',
          name: 'DashboardProvider');
      developer.log(
          'Monitoring - Total: $_totalMonitoring, Unsynced: $_unsyncedMonitoring',
          name: 'DashboardProvider');
      developer.log('Audit - Total: $_totalAudit, Unsynced: $_unsyncedAudit',
          name: 'DashboardProvider');
      developer.log('Last Training Site Sync: $_lastTrainingSiteSync',
          name: 'DashboardProvider');
    } catch (e) {
  ...
  Future<void> _syncReferenceData() async {
    try {
      developer.log('========================================',
          name: 'DashboardProvider');
      developer.log('Syncing reference data (districts & authorities)...',
          name: 'DashboardProvider');
      ...
      if (districtsResult.success) {
        developer.log(
            '✅ Synced ${districtsResult.data ?? 0} districts to local database',
            name: 'DashboardProvider');
      } else {
        developer.log('⚠️ Failed to sync districts: ${districtsResult.message}',
            name: 'DashboardProvider');
      }

      if (authoritiesResult.success) {
        developer.log(
            '✅ Synced ${authoritiesResult.data ?? 0} authorities to local database',
            name: 'DashboardProvider');
      } else {
        developer.log(
            '⚠️ Failed to sync authorities: ${authoritiesResult.message}',
            name: 'DashboardProvider');
      }
      ...
      developer.log('Error syncing reference data: $e',
          name: 'DashboardProvider');
  ...
  Future<void> _loadLastSyncTimes() async {
    try {
      ...
      _lastMonitoringSync = monitoringTimestamp != null
          ...
          : null;
      _lastAuditSync =
          auditTimestamp != null ? DateTime.parse(auditTimestamp) : null;

      developer.log('Loaded sync times from preferences',
          name: 'DashboardProvider');
    } catch (e) {
  ...
  Future<void> _saveLastSyncTime(String moduleType, DateTime syncTime) async {
    try {
      ...
      await prefs.setString(
          'last_${moduleType}_sync', syncTime.toIso8601String());
      ...
      developer.log('Saved $moduleType sync time: $syncTime',
          name: 'DashboardProvider');
      notifyListeners();
    } catch (e) {
      developer.log('Error saving sync time for $moduleType: $e',
          name: 'DashboardProvider');
    }
  }
  ...
  Future<bool> isInitialSyncCompleted() async {
    try {
      ...
      developer.log('Error checking initial sync status: $e',
          name: 'DashboardProvider');
      return false;
    }
  }
  ...
  Future<void> markInitialSyncCompleted() async {
    try {
      ...
      developer.log('Marked initial sync as completed',
          name: 'DashboardProvider');
    } catch (e) {
      developer.log('Error marking initial sync completed: $e',
          name: 'DashboardProvider');
    }
  }
  ...
  Future<SyncResult> performInitialSync({
    ...
    try {
      ...
      final checkpoint = await _dataService.getTrainingSiteSyncCheckpoint();
      final resuming =
          checkpoint['hasCheckpoint'] == true && restartFromFirstPage == false;

      if (resuming) {
        onProgress?.call(
          'Resuming from page ${checkpoint['nextPage']}/${checkpoint['totalPages']}...',
          checkpoint['processedRecords'] as int,
          checkpoint['totalRecords'] as int,
        );
      }
      ...
      final response = await _dataService.getAllTrainingSitesPaginated(
        ...
        onProgress: (currentRecords, totalRecords, currentPage, totalPages) {
          // Report progress with actual record counts
          onProgress?.call('Processing page $currentPage/$totalPages...',
      ...
      if (response.success && response.data != null) {
        final recordsProcessed =
            response.data!['recordsProcessed'] as int? ?? 0;
        ...
        developer.log('========================================',
            name: 'DashboardProvider');
        developer.log('✅ Successfully synced $recordsProcessed training sites',
            name: 'DashboardProvider');
        ...
        onProgress?.call(
            'Initial sync completed', recordsProcessed, recordsProcessed);

        return SyncResult(
          success: true,
          recordsProcessed: recordsProcessed,
          message:
              'Initial sync completed successfully! Synced $recordsProcessed training sites.',
        );
      } else {
        final checkpoint = await _dataService.getTrainingSiteSyncCheckpoint();
        final pageInfo = checkpoint['hasCheckpoint'] == true
            ? ' Last processed: ${checkpoint['processedRecords']}/${checkpoint['totalRecords']} (next page ${checkpoint['nextPage']}).'
            : '';
        ...
        return SyncResult(
          success: false,
          message:
              '${response.message ?? 'Failed to perform initial sync'}$pageInfo',
        );
      }
    } catch (e) {
      developer.log('========================================',
  ...
  Future<SyncResult> performIncrementalSync({
    ...
    try {
      ...
      if (_lastTrainingSiteSync == null) {
        developer.log('No last sync time found, falling back to initial sync',
            name: 'DashboardProvider');
      ...
      developer.log('Last sync date (ISO 8601): $lastSyncDate',
          name: 'DashboardProvider');
      developer.log(
          'New sync time will be: ${newSyncTime.toUtc().toIso8601String()}',
          name: 'DashboardProvider');
      onProgress?.call('Checking for updates since $lastSyncDate...', 0, 0);

      // Fetch updated training sites
      final response =
          await _dataService.syncUpdatedTrainingSites(lastSyncDate);
      ...
      if (response.success) {
        ...
        developer.log('========================================',
            name: 'DashboardProvider');
        developer.log(
            '✅ Successfully synced $recordsProcessed updated training sites',
            name: 'DashboardProvider');
        ...
        onProgress?.call(
            'Incremental sync completed', recordsProcessed, recordsProcessed);
  ...
  Future<SyncResult> syncTrainingSites({
    ...
    try {
      ...
      if (_lastTrainingSiteSync == null) {
        print('No last sync time found, performing initial full sync...');
        developer.log(
            'No last sync time found, performing initial full sync...',
            name: 'DashboardProvider');
        ...
        developer.log(
            'Last sync time found: $_lastTrainingSiteSync, performing incremental sync...',
            name: 'DashboardProvider');
        return await performIncrementalSync(onProgress: onProgress);
      }
    } catch (e) {
      developer.log('Error in syncTrainingSites: $e',
          name: 'DashboardProvider');
      return SyncResult(
        success: false,
  ...
  Future<SyncResult> syncTrainingSitesLegacy({
    ...
    try {
      developer.log('========================================',
          name: 'DashboardProvider');
      developer.log('Starting training sites sync...',
          name: 'DashboardProvider');
      developer.log('========================================',
          name: 'DashboardProvider');
      ...
      print(
          '[DashboardProvider] syncTrainingSitesLegacy called. page=$currentPage, limit=100');

      developer.log('📄 Fetching page: $currentPage',
          name: 'DashboardProvider');
      ...
      if (result.success && result.data != null) {
        ...
        developer.log('========================================',
            name: 'DashboardProvider');
        developer.log('📄 Current Page: ${paginatedData.currentPage}',
            name: 'DashboardProvider');
        developer.log('📄 Total Pages: ${paginatedData.totalPages}',
            name: 'DashboardProvider');
        developer.log('📦 Records in this page: $recordsProcessed',
            name: 'DashboardProvider');
        developer.log(
            '📊 Total Records on server: ${paginatedData.totalRecords}',
            name: 'DashboardProvider');
        developer.log('📏 Limit: ${paginatedData.limit}',
            name: 'DashboardProvider');
        ...
        if (paginatedData.data.isNotEmpty) {
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('📋 SAMPLE RECORDS (first 3):',
              name: 'DashboardProvider');
          developer.log('========================================',
              name: 'DashboardProvider');

          final sampleCount =
              paginatedData.data.length > 3 ? 3 : paginatedData.data.length;
          for (int i = 0; i < sampleCount; i++) {
            final site = paginatedData.data[i];
            developer.log('Record ${i + 1}:', name: 'DashboardProvider');
            developer.log('  - ID: ${site.trainingPointId}',
                name: 'DashboardProvider');
            developer.log('  - Name: ${site.trainingSite}',
                name: 'DashboardProvider');
            developer.log('  - District: ${site.district}',
                name: 'DashboardProvider');
            developer.log('  - Authority: ${site.traditionalAuthority}',
                name: 'DashboardProvider');
            developer.log('  - Latitude: ${site.latitude}',
                name: 'DashboardProvider');
            developer.log('  - Longitude: ${site.longitude}',
                name: 'DashboardProvider');
            developer.log('  - Households: ${site.houseHoldsCount}',
                name: 'DashboardProvider');
            developer.log('  - Total People: ${site.totalPeople}',
                name: 'DashboardProvider');
            developer.log('  - Cookstoves: ${site.cookstovesCount}',
                name: 'DashboardProvider');
        ...
        if (paginatedData.data.isNotEmpty) {
          try {
            ...
            developer.log(
                '📝 Marking $recordsProcessed records as synced (s_is_sync = 1)',
                name: 'DashboardProvider');

            await _trainingSiteRepository.insertBulk(syncedRecords);

            developer.log(
                '✅ Successfully stored $recordsProcessed training sites in local database',
                name: 'DashboardProvider');
          } catch (e) {
            developer.log('❌ Error storing training sites: $e',
                name: 'DashboardProvider');
          }
        }
        ...
        if (isComplete) {
          ...
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('✅ All pages fetched successfully',
              name: 'DashboardProvider');
          developer.log('🔄 Reset page counter to 1 for next sync',
              name: 'DashboardProvider');
          ...
          return SyncResult(
            ...
            recordsProcessed: recordsProcessed,
            message:
                'Successfully synced $recordsProcessed records. All data is up to date!',
          );
        } else {
          ...
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('📄 Next page will be: ${currentPage + 1}',
              name: 'DashboardProvider');
          developer.log(
              '📊 Remaining pages: ${paginatedData.totalPages - currentPage}',
              name: 'DashboardProvider');
          ...
          return SyncResult(
            ...
            recordsProcessed: recordsProcessed,
            message:
                'Fetched $recordsProcessed records (Page ${paginatedData.currentPage}/${paginatedData.totalPages}). Records are still pending on the server.',
          );
        }
      } else {
        developer.log('========================================',
      ...
      if (e.toString().contains('has no column named')) {
        developer.log(
            '⚠️  Database schema issue detected, attempting to fix...',
            name: 'DashboardProvider');
        ...
        try {
          ...
          if (schemaResult.success) {
            ...
            return SyncResult(
              success: false,
              message:
                  'Database schema issue detected. Please restart the app and try again.',
            );
          }
        } catch (schemaError) {
          return SyncResult(
            success: false,
            message:
                'Database schema issue. Please restart the app and try again.',
          );
        }
      }
  ...
  Future<void> resetTrainingSitesPageCounter() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setInt('training_sites_current_page', 1);
      developer.log('Reset training sites page counter to 1',
          name: 'DashboardProvider');
    } catch (e) {
      developer.log('Error resetting page counter: $e',
          name: 'DashboardProvider');
    }
  }

  Future<Map<String, dynamic>> getTrainingSiteSyncCheckpointInfo() async {
    return _dataService.getTrainingSiteSyncCheckpoint();
  }

  Future<void> resetTrainingSiteSyncCheckpoint() async {
    await _dataService.clearTrainingSiteSyncCheckpoint();
  }
  ...
  Future<SyncResult> syncMonitoring({
    ...
    try {
      ...
      if (isInitialSync) {
        developer.log('Performing INITIAL FULL SYNC using pagination',
            name: 'DashboardProvider');
        ...
        final response = await _dataService.getAllMonitoringPaginated(
          ...
          onProgress: (currentRecords, totalRecords, currentPage, totalPages) {
            // Report progress with actual record counts
            onProgress?.call('Downloading page $currentPage/$totalPages...',
                currentRecords, totalRecords);
          },
          storeInDatabase: true,
          restartFromFirstPage: restartFromFirstPage,
        ...
        if (response.success && response.data != null) {
          final recordsProcessed = response.data!['recordsProcessed'] as int;

          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('INITIAL MONITORING SYNC COMPLETED',
              name: 'DashboardProvider');
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log(
          ...
          onProgress?.call(
              'Monitoring sync completed', recordsProcessed, recordsProcessed);
          ...
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('INITIAL MONITORING SYNC FAILED',
              name: 'DashboardProvider');
          developer.log('========================================',
              name: 'DashboardProvider');
        ...
        developer.log(
            'Performing INCREMENTAL SYNC using monitoring_data endpoint',
            name: 'DashboardProvider');
        ...
        if (response.success) {
          ...
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('INCREMENTAL MONITORING SYNC COMPLETED',
              name: 'DashboardProvider');
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log(
          ...
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('INCREMENTAL MONITORING SYNC FAILED',
              name: 'DashboardProvider');
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('Error: ${response.message}',
          ...
          return SyncResult(
            success: false,
            message:
                response.message ?? 'Failed to sync updated monitoring records',
          );
        }
      }
    } catch (e) {
    ...
    try {
      ...
      if (isInitialSync) {
        ...
        if (response.success && response.data != null) {
          ...
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('INITIAL AUDIT SYNC COMPLETED',
              name: 'DashboardProvider');
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('✅ Successfully synced $recordsProcessed audit records',
              name: 'DashboardProvider');
          ...
          onProgress?.call(
              'Audit sync completed', recordsProcessed, recordsProcessed);
        ...
        developer.log('Performing INCREMENTAL SYNC using audit_data endpoint',
            name: 'DashboardProvider');
        ...
        if (response.success) {
          ...
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('INCREMENTAL AUDIT SYNC COMPLETED',
              name: 'DashboardProvider');
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log(
          ...
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('INCREMENTAL AUDIT SYNC FAILED',
              name: 'DashboardProvider');
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('Error: ${response.message}',
  ...
  Future<Map<String, dynamic>> getMonitoringSyncCheckpointInfo() async {
    return _dataService.getMonitoringSyncCheckpoint();
  }

  Future<void> resetMonitoringSyncCheckpoint() async {
    await _dataService.clearMonitoringSyncCheckpoint();
  }

  Future<Map<String, dynamic>> getAuditSyncCheckpointInfo() async {
    return _dataService.getAuditSyncCheckpoint();
  }

  Future<void> resetAuditSyncCheckpoint() async {
    await _dataService.clearAuditSyncCheckpoint();
  }
  ...
  Future<void> testDatabaseQueries() async {
    try {
      ...
      final syncedTrainingSiteCount =
          await _trainingSiteRepository.getSyncedCount();
      final unsyncedTrainingSiteCount =
          await _trainingSiteRepository.getUnsyncedCount();
      final beneficiaryCount = await _beneficiaryRepository.getCount();
      final unsyncedBeneficiaryCount =
          await _beneficiaryRepository.getUnsyncedCount();

      developer.log('Manual Test Results:', name: 'DashboardProvider');
      developer.log(
          'Training Sites - Total: $trainingSiteCount, Synced: $syncedTrainingSiteCount, Unsynced: $unsyncedTrainingSiteCount',
          name: 'DashboardProvider');
      developer.log(
          'Beneficiaries: $beneficiaryCount (Unsynced: $unsyncedBeneficiaryCount)',
          name: 'DashboardProvider');
    } catch (e) {
  ...
  Future<SyncResult> syncBeneficiaries({
    ...
    try {
      ...
      if (isInitialSync) {
        ...
        final checkpoint = await _dataService.getBeneficiarySyncCheckpoint();
        final resuming = checkpoint['hasCheckpoint'] == true &&
        ...
        if (response.success && response.data != null) {
          ...
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('INITIAL BENEFICIARY SYNC COMPLETED',
              name: 'DashboardProvider');
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('✅ Successfully synced $recordsProcessed beneficiaries',
              name: 'DashboardProvider');
          ...
          onProgress?.call(
              'Beneficiary sync completed', recordsProcessed, recordsProcessed);
          ...
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('INITIAL BENEFICIARY SYNC FAILED',
              name: 'DashboardProvider');
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('Error: ${response.message}',
          ...
          return SyncResult(
            success: false,
            message:
                '${response.message ?? 'Failed to sync beneficiaries'}$pageInfo',
          );
        }
      } else {
        developer.log(
            'Performing INCREMENTAL SYNC using Beneficiary_data endpoint',
            name: 'DashboardProvider');
        ...
        // Use incremental sync endpoint
        final response =
            await _dataService.syncUpdatedBeneficiaries(lastSyncDate);
        ...
        if (response.success) {
          ...
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('INCREMENTAL BENEFICIARY SYNC COMPLETED',
              name: 'DashboardProvider');
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log(
          ...
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('INCREMENTAL BENEFICIARY SYNC FAILED',
              name: 'DashboardProvider');
          developer.log('========================================',
              name: 'DashboardProvider');
          developer.log('Error: ${response.message}',
  ...
  Future<Map<String, dynamic>> getBeneficiarySyncCheckpointInfo() async {
    return _dataService.getBeneficiarySyncCheckpoint();
  }

  Future<void> resetBeneficiarySyncCheckpoint() async {
    await _dataService.clearBeneficiarySyncCheckpoint();
  }
}
