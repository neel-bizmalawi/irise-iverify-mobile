...
class _AuditListScreenState extends State<AuditListScreen> {
  ...
  void initState() {
    ...
    _loadAudits();
    _loadUnsyncedCount();
    _searchController.addListener(() => _filterAudits(_searchController.text));
  }
  ...
  Future<void> _loadAudits() async {
    ...
    try {
      ...
      developer.log('Loaded ${audits.length} audits from local database', name: 'AuditList');
  ...
  Future<void> _syncAuditToServer(Audit audit) async {
    if (audit.auditId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Cannot sync audit without ID'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    try {
      developer.log('Syncing audit ${audit.auditId} to server...', name: 'AuditList');
      ...
      if (audit.photoPathCookStove != null && audit.photoPathCookStove!.isNotEmpty) {
      ...
      if (audit.photoPathCookStoveArea != null && audit.photoPathCookStoveArea!.isNotEmpty) {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        await _auditRepository.markAsSynced(audit.auditId!);
        
        developer.log('Successfully synced audit ${audit.auditId}', name: 'AuditList');
        ...
        children: [
          ...
          Positioned(
            ...
            child: FloatingActionButton.extended(
              ...
              label: const Text('Add Audit', style: TextStyle(color: Colors.white)),
            ),
          ),
        ],
      ),
