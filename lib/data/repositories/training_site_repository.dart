...
class TrainingSiteRepository {
    ...
    try {
      ...
      if (existing != null) {
        ...
        if (existingSIsSync == 0 && trainingSite.sIsSync == 1) {
          ...
          developer.log(
              'Skipping insert - existing record has local unsynced changes: ${trainingSite.trainingSite}',
              name: 'TrainingSiteRepo');
          return existing['offline_id'] as int;
        }

        // Record exists - update it instead
        developer.log(
            'Training site already exists, updating instead: ${trainingSite.trainingSite}',
            name: 'TrainingSiteRepo');
        await db.update(
          _tableName,
          trainingSite.toMap(),
        ...
        developer.log('Inserted training site with id: $id',
            name: 'TrainingSiteRepo');
        return id;
      }
    } catch (e) {
      developer.log('Error inserting training site: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<void> insertBulk(List<TrainingSite> trainingSites) async {
    try {
      ...
      developer.log('========================================',
          name: 'TrainingSiteRepo');
      developer.log(
          'BULK INSERT: Processing ${trainingSites.length} training sites',
          name: 'TrainingSiteRepo');
      developer.log('========================================',
          name: 'TrainingSiteRepo');
      ...
      developer.log('Records in DB BEFORE bulk insert: $recordsBefore',
          name: 'TrainingSiteRepo');

      // Get count of unsynced records BEFORE bulk insert
      final unsyncedBefore = await db.rawQuery(
          'SELECT COUNT(*) as count FROM $_tableName WHERE s_is_sync = 0');
      final unsyncedCountBefore = Sqflite.firstIntValue(unsyncedBefore) ?? 0;
      developer.log('Unsynced records BEFORE bulk insert: $unsyncedCountBefore',
          name: 'TrainingSiteRepo');
      ...
      final testRecordsBefore = await db.rawQuery(
          ...
          ['t', 'tt', 'ttt']);
      developer.log('>>> Test records BEFORE bulk insert:',
          name: 'TrainingSiteRepo');
      for (var record in testRecordsBefore) {
        developer.log(
            '>>>   ${record['training_site']}: offline_id=${record['offline_id']}, training_point_id=${record['training_point_id']}, s_is_sync=${record['s_is_sync']}',
            name: 'TrainingSiteRepo');
      }

      // CRITICAL FIX: Separate incoming records into synced (from server) and unsynced (local)
      final syncedIncoming =
          trainingSites.where((site) => site.sIsSync == 1).toList();
      final unsyncedIncoming =
          trainingSites.where((site) => site.sIsSync == 0).toList();

      developer.log(
          'Incoming: ${syncedIncoming.length} synced (from server), ${unsyncedIncoming.length} unsynced (local)',
          name: 'TrainingSiteRepo');
      ...
      for (var site in trainingSites) {
        try {
          ...
          final isTestRecord = site.trainingSite == 't' ||
              site.trainingSite == 'tt' ||
              site.trainingSite == 'ttt';

          if (isTestRecord) {
            developer.log(
                '>>> PROCESSING TEST RECORD: ${site.trainingSite} (training_point_id: ${site.trainingPointId}, offline_id: ${site.offlineId}, s_is_sync: ${site.sIsSync})',
                name: 'TrainingSiteRepo');
          }
          ...
          if (existing != null) {
            // Check if existing record is unsynced (local changes)
            final existingSIsSync = existing['s_is_sync'] as int? ?? 0;
            final existingTrainingPointId =
                existing['training_point_id'] as int?;
            final existingOfflineId = existing['offline_id'] as int?;
            final incomingTrainingPointId = site.trainingPointId;

            if (isTestRecord) {
              developer.log(
                  '>>> Found existing: offline_id=${existing['offline_id']}, training_point_id=$existingTrainingPointId, s_is_sync=$existingSIsSync',
                  name: 'TrainingSiteRepo');
            }
            ...
            final siteToUpdate =
                (site.offlineId == null && existingOfflineId != null)
            ...
            if (existingSIsSync == 0 && site.sIsSync == 1) {
              ...
              if (isTestRecord) {
                developer.log(
                    '>>> ACTION: SKIP (protecting local unsynced changes)',
                    name: 'TrainingSiteRepo');
              }
            ...
            if (existingSIsSync == 0 &&
                incomingTrainingPointId != null &&
                existingTrainingPointId == null) {
              ...
              if (existingOfflineId != null &&
                  existingOfflineId != incomingTrainingPointId) {
                await _beneficiaryRepository.remapTrainingSiteId(
                  existingOfflineId,
                  incomingTrainingPointId,
                );
              }
              updated++;
              if (isTestRecord) {
                developer.log(
                    '>>> ACTION: UPDATE (local record synced to server, preserved offline_id: $existingOfflineId)',
                    name: 'TrainingSiteRepo');
              }
              ...
              if (isTestRecord) {
                developer.log(
                    '>>> ACTION: UPDATE (both synced, preserved offline_id: $existingOfflineId)',
                    name: 'TrainingSiteRepo');
              }
            } else {
              ...
              if (isTestRecord) {
                developer.log(
                    '>>> ACTION: SKIP (other edge case: existingSIsSync=$existingSIsSync, incomingSIsSync=${site.sIsSync})',
                    name: 'TrainingSiteRepo');
              }
            }
          } else {
            ...
            if (insertResult > 0) {
              ...
              if (isTestRecord) {
                developer.log(
                    '>>> ACTION: INSERT (new record, id=$insertResult)',
                    name: 'TrainingSiteRepo');
              }
            } else {
              skipped++;
              if (isTestRecord) {
                developer.log(
                    '>>> ACTION: SKIP (insert conflict, record already exists)',
                    name: 'TrainingSiteRepo');
              }
            }
          }
        } catch (e) {
          developer.log('ERROR processing site ${site.trainingSite}: $e',
              name: 'TrainingSiteRepo');
        }
      }
      ...
      developer.log('Records in DB AFTER bulk insert: $recordsAfter',
          name: 'TrainingSiteRepo');
      developer.log(
          'Change: ${recordsAfter - recordsBefore} (inserted: $inserted, updated: $updated, skipped: $skipped)',
          name: 'TrainingSiteRepo');

      // Get count of unsynced records AFTER bulk insert
      final unsyncedAfter = await db.rawQuery(
          'SELECT COUNT(*) as count FROM $_tableName WHERE s_is_sync = 0');
      final unsyncedCountAfter = Sqflite.firstIntValue(unsyncedAfter) ?? 0;
      developer.log('Unsynced records AFTER bulk insert: $unsyncedCountAfter',
          name: 'TrainingSiteRepo');

      // CRITICAL CHECK: Verify no local unsynced records were lost
      if (unsyncedCountAfter < unsyncedCountBefore) {
        developer.log(
            '⚠️ WARNING: Lost ${unsyncedCountBefore - unsyncedCountAfter} unsynced local records!',
            name: 'TrainingSiteRepo');
      } else {
      ...
      final testRecordsAfter = await db.rawQuery(
          'SELECT offline_id, training_point_id, training_site, s_is_sync FROM $_tableName WHERE training_site IN (?, ?, ?)',
          ['t', 'tt', 'ttt']);
      developer.log('>>> Test records AFTER bulk insert:',
          name: 'TrainingSiteRepo');
      ...
      developer.log('Inserted: $inserted, Updated: $updated, Skipped: $skipped',
          name: 'TrainingSiteRepo');
      developer.log('========================================',
          name: 'TrainingSiteRepo');
    } catch (e) {
      developer.log('Error bulk inserting training sites: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>?> _findExisting(
      Database db, TrainingSite site) async {
    try {
      ...
      if (site.trainingPointId != null) {
        ...
        if (results.isNotEmpty) {
          if (isTestRecord) {
            developer.log(
                '>>> _findExisting: Found ${site.trainingSite} by training_point_id: ${site.trainingPointId}',
                name: 'TrainingSiteRepo');
          }
          return results.first;
        } else if (isTestRecord) {
          developer.log(
              '>>> _findExisting: NOT found ${site.trainingSite} by training_point_id: ${site.trainingPointId}',
              name: 'TrainingSiteRepo');
        }
      }
      ...
      if (site.offlineId != null) {
        ...
        if (results.isNotEmpty) {
          if (isTestRecord) {
            developer.log(
                '>>> _findExisting: Found ${site.trainingSite} by offline_id: ${site.offlineId}',
                name: 'TrainingSiteRepo');
          }
          return results.first;
        } else if (isTestRecord) {
          developer.log(
              '>>> _findExisting: NOT found ${site.trainingSite} by offline_id: ${site.offlineId}',
              name: 'TrainingSiteRepo');
        }
      }
      ...
      if (isTestRecord) {
        developer.log(
            '>>> _findExisting: NO MATCH found for ${site.trainingSite} (training_point_id: ${site.trainingPointId}, offline_id: ${site.offlineId})',
            name: 'TrainingSiteRepo');
      }
      return null;
    } catch (e) {
      developer.log('Error finding existing record: $e',
          name: 'TrainingSiteRepo');
      return null;
    }
  }
  ...
  Future<List<TrainingSite>> getAll() async {
    try {
      ...
      developer.log('========================================',
          name: 'TrainingSiteRepo');
      developer.log(
          'GET ALL: Retrieved ${maps.length} training sites from local DB',
          name: 'TrainingSiteRepo');
      ...
      for (int i = 0; i < sampleCount; i++) {
        final record = maps[i];
        developer.log(
            '  Record ${i + 1}: ${record['training_site']} (offline_id=${record['offline_id']}, training_point_id=${record['training_point_id']}, s_is_sync=${record['s_is_sync']})',
            name: 'TrainingSiteRepo');
      }
      developer.log('========================================',
          name: 'TrainingSiteRepo');
      ...
      developer.log('Error getting all training sites: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<TrainingSite?> getById(int id) async {
    try {
      ...
      developer.log(
          'Found training site: ${maps.first['training_site']} (training_point_id: ${maps.first['training_point_id']}, offline_id: ${maps.first['offline_id']})',
          name: 'TrainingSiteRepo');
      return TrainingSite.fromMap(maps.first);
    } catch (e) {
      developer.log('Error getting training site by id: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<TrainingSite?> getByTrainingPointId(int trainingPointId) async {
    try {
      ...
      developer.log('Error getting training site by training_point_id: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<TrainingSite?> getByOfflineId(int offlineId) async {
    try {
      ...
      developer.log('Error getting training site by offline_id: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<List<TrainingSite>> getActive() async {
    try {
      ...
      developer.log('Error getting active training sites: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<List<TrainingSite>> getUnsynced() async {
    try {
      ...
      developer.log('Error getting unsynced training sites: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<List<TrainingSite>> search(String query) async {
    try {
      ...
      developer.log('Error searching training sites: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<int> update(TrainingSite trainingSite) async {
    try {
      ...
      if (trainingSite.trainingPointId != null) {
        ...
        if (count > 0) {
          developer.log(
              'Updated training site by training_point_id: ${trainingSite.trainingPointId}',
              name: 'TrainingSiteRepo');
          return count;
        }
      }
      ...
      if (trainingSite.offlineId != null) {
        ...
        if (count > 0) {
          developer.log(
              'Updated training site by offline_id: ${trainingSite.offlineId}',
              name: 'TrainingSiteRepo');
          return count;
        }
      }
      ...
      if (existing != null) {
        ...
        developer.log(
            'Updated training site by offline_id: ${existing['offline_id']}',
            name: 'TrainingSiteRepo');
        return count;
      }

      developer.log('No training site found to update',
          name: 'TrainingSiteRepo');
      return 0;
    } catch (e) {
      developer.log('Error updating training site: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<int> markAsSynced(int id) async {
    try {
      ...
      developer.log('Error marking training site as synced: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<int> delete(int id) async {
    try {
      ...
      developer.log('Error deleting training site: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<int> softDelete(int id) async {
    try {
      ...
      developer.log('Error soft deleting training site: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<int> getCount() async {
    try {
      ...
      developer.log('Error getting training sites count: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<int> getUnsyncedCount() async {
    try {
      ...
      developer.log('Error getting unsynced training sites count: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<int> getSyncedCount() async {
    try {
      final db = await _dbHelper.database;
      final result = await db.rawQuery(
          'SELECT COUNT(*) as count FROM $_tableName WHERE s_is_sync = 1');
      return Sqflite.firstIntValue(result) ?? 0;
    } catch (e) {
      developer.log('Error getting synced training sites count: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<bool> areAllTrainingSitesSynced() async {
    try {
      ...
      if (totalCount == 0) {
        developer.log('No training sites found in database',
            name: 'TrainingSiteRepo');
        return false;
      }
      ...
      developer.log(
          'Training sites sync status: $unsyncedCount unsynced out of $totalCount total',
          name: 'TrainingSiteRepo');

      return allSynced;
    } catch (e) {
      developer.log('Error checking if all training sites are synced: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<void> clearAll() async {
    try {
      ...
      developer.log('Error clearing training sites: $e',
          name: 'TrainingSiteRepo');
      rethrow;
    }
  }
  ...
  Future<int> getNextOfflineId() async {
    try {
      final db = await _dbHelper.database;
      final result = await db.rawQuery(
          'SELECT MAX(offline_id) as max_id FROM $_tableName WHERE offline_id IS NOT NULL');
      final maxId = Sqflite.firstIntValue(result) ?? 0;
      return maxId + 1;
    } catch (e) {
      developer.log('Error getting next offline ID: $e',
          name: 'TrainingSiteRepo');
